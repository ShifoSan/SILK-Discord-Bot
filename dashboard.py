...
async def get_chat_config(guild_id):
    ...
    config = await app.chat_configs.find_one({"guild_id": guild_id}, {"_id": 0})
    if config:
        return jsonify(config), 200
    else:
        # Return default if not found
        return jsonify({"guild_id": guild_id, "enabled": False, "language": "English"}), 200
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=2160)        app.db_client = motor.motor_asyncio.AsyncIOMotorClient(MONGO_URI, tlsCAFile=certifi.where())
    else:
        app.db_client = motor.motor_asyncio.AsyncIOMotorClient("mongodb://localhost:27017/")
    app.db = app.db_client.silk_bot
    app.chat_configs = app.db.chat_configs
