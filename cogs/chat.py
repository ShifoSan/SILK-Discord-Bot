...
class Chat(commands.Cog):
    ...
    async def cog_load(self):
        ...
        async for config in self.chat_configs.find():
            if "guild_id" in config:
                self.channel_configs[config["guild_id"]] = config
            elif "channel_id" in config:
                self.channel_configs[config["channel_id"]] = config

    async def update_channel_config(self, guild_id: int, updates: dict):
        if guild_id not in self.channel_configs:
            self.channel_configs[guild_id] = {"guild_id": guild_id, "enabled": False, "language": "English"}
        self.channel_configs[guild_id].update(updates)
        await self.chat_configs.update_one(
            {"guild_id": guild_id},
            {"$set": self.channel_configs[guild_id]},
            upsert=True
        )
    ...
    async def chat_toggle(self, interaction: discord.Interaction, state: bool, language: app_commands.Choice[str] = None):
        ...
        if state:
            ...
            await interaction.response.send_message("❌ Auto-Chat Disabled for this server.", ephemeral=True)
    ...
    async def on_message(self, message: discord.Message):
        ...
        channel_config = self.channel_configs.get(guild_id, {"enabled": False, "language": "English"})
